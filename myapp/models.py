...
class CustomUserManager(BaseUserManager):
    def create_user(self, email, hotel_name, password=None, **extra_fields):
        if not email:
            raise ValueError("The Email field must be set")
        if not hotel_name:
            raise ValueError("The Hotel name field must be set")

        email = self.normalize_email(email)
        user = self.model(email=email, hotel_name=hotel_name, **extra_fields)
        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, email, hotel_name, password=None, **extra_fields):
        extra_fields.setdefault("is_staff", True)
        extra_fields.setdefault("is_superuser", True)
        extra_fields.setdefault("is_active", True)
        ...
        return self.create_user(email, hotel_name, password, **extra_fields)
